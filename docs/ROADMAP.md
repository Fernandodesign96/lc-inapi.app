...
- [x] **`/auditar` — ingreso de URL, atajos, inventarios en barras colapsables:** **barra principal** de ingreso de URL (mismos dominios y validación que hoy). **Debajo** del bloque de ingreso, sección de **tres atajos** editoriales (peor / intermedio / mejor LC) hacia **`/auditar/procesando?url=…`** y resultado. Las **listas seccionadas** de apoyo conviven en **barras colapsables** (§15 design system). **Objetivo documentado (2026-05-28, actualizado):** **un** acordeón de inventario LC — **Historial de Auditoría LC - URLs INAPI** (~20 URLs Clarity; tabla con Encargado, Auditorías, Última revisión, % LC, Estado; fuente [`data/ux/clarity-fichas-mock.json`](../data/ux/clarity-fichas-mock.json)) — más ficha `/auditar/inventario/clarity/[rank]`. Suprimidos en UI: «URLs más auditadas» y «URLs con estados LC resueltos» / «Estados URLs». Referencia: [`docs/ux/inventario-urls-clarity.md`](ux/inventario-urls-clarity.md).
...
- [ ] **Feedback UX post-demo (mayo 2026) — inventarios consistentes:** unificar mock bajo [`data/ux/clarity-fichas-mock.json`](../data/ux/clarity-fichas-mock.json); **fusionar** columnas de «URLs más auditadas» en la tabla **20 URLs Clarity** (Encargado, Auditorías, Última revisión); **eliminar** acordeón «URLs con estados LC resueltos» / «Estados URLs» en favor del **Historial de Auditoría LC - URLs INAPI** (observaciones en ficha); iconografía LC **! / ✓ / ✓✓ / —** y color de fila por umbrales ≤80 / 81–90 / ≥91 % en tabla de historial y ficha; historial de ficha alineado a conteo de auditorías (p. ej. rank 1 → 5). **Documentación:** [`docs/ux/inventario-urls-clarity.md`](ux/inventario-urls-clarity.md), [`docs/DESIGN_SYSTEM.md`](DESIGN_SYSTEM.md) §13.1, devlog 2026-05-28. *(Implementación en código: fusión «más auditadas» hecha; pendiente 2c eliminar estados resueltos + filtros Etapa 5.)*
...
- [ ] **Feedback UX — Etapa 3 (base en repo):** ruta `/auditar/inventario/clarity/[rank]` + enlaces; cerrar tras consistencia de datos/UI.
- [ ] **Feedback UX — Etapa 4:** sección mock «Calidad web (Sitio Web)» en acordeón.
- [ ] **Feedback UX — Etapa 5:** filtros y orden cliente en la **tabla única** de historial LC (estado; visitas; auditorías; última revisión; % LC — sin filtro por encargado ni observación).

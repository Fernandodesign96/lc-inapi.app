...
- [x] **`/auditar` — ingreso de URL, atajos, inventarios en barras colapsables:** **barra principal** de ingreso de URL (mismos dominios y validación que hoy). **Debajo** del bloque de ingreso, sección de **tres atajos** editoriales (peor / intermedio / mejor LC) hacia **`/auditar/procesando?url=…`** y resultado. Las **listas seccionadas** de apoyo conviven en **barras colapsables** (§15 design system). **Objetivo documentado (2026-05-28):** **dos** acordeones de inventario — (1) **~20 URLs Clarity** (tabla unificada con Encargado, Auditorías, Última revisión, % LC, Estado; fuente [`data/ux/clarity-fichas-mock.json`](../data/ux/clarity-fichas-mock.json)), (2) **Estados URLs** — más ficha `/auditar/inventario/clarity/[rank]`. La sección «URLs más auditadas» queda **suprimida** en favor de columnas en la tabla Clarity. Referencia: [`docs/ux/inventario-urls-clarity.md`](ux/inventario-urls-clarity.md).
...
- [ ] **Feedback UX — Etapa 5:** filtros cliente en las **dos** tablas de inventario (Clarity + Estados URLs).

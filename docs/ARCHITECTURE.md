...
- En **`/auditar`**, **un** acordeón de inventario LC mock — **Historial de Auditoría LC - URLs INAPI** (tabla **20 URLs** Clarity; fuente [`data/ux/clarity-fichas-mock.json`](../data/ux/clarity-fichas-mock.json)) — más rutas de ficha **`/auditar/inventario/clarity/[rank]`** (observaciones y historial por URL); patrón colapsable (título + flecha abajo + panel), ver [`docs/DESIGN_SYSTEM.md`](DESIGN_SYSTEM.md) §15. Inventario documental: [`docs/ux/inventario-urls-clarity.md`](ux/inventario-urls-clarity.md).
